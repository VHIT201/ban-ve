...
const StatsCards = ({ period, onPeriodChange }: StatsCardsProps) => {
  const { data, isLoading } = useGetApiAdminStats({ period })
    ...
    <div className='space-y-6'>
      ...
      <div className='flex items-center gap-3'>
        ...
        <Button
          ...
          className={cn(
            'transition-all',
            period === 'quarter' && 'bg-green-500 hover:bg-green-600 text-white'
          )}
        >
          3 Tháng
        </Button>
      ...
      <div className='flex items-center gap-8'>
        {isLoading ? (
          ...
          Array.from({ length: 4 }).map((_, idx) => (
            <Card key={idx} className='flex-1 border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800 animate-pulse'>
          ...
          STAT_CONFIGS.map((config) => {
            ...
            return (
              <Card key={config.key} className='flex-1 border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800'>
