...
import { AutoCompleteFieldConfig, FieldConfig, Props, SelectFieldConfig, SelectQueryFieldConfig } from './lib/types'
import DateRangerPicker from '../date-ranger-picker/DateRangerPicker'
import { getDefaultValuesFromSchema, getSchemaShape, unwrapSchema } from './lib/utils'
import AutoComplete from '../auto-complete'
import { AutoCompleteContent } from '../auto-complete/shared'

// Component
const DynamicFilter = <T extends z.ZodTypeAny>(props: Props<T>) => {
  const { schema, onSubmit, defaultValues, fieldConfig = {} as Record<string, FieldConfig> } = props
  ...
  const renderFormField = (
    ...
    switch (fieldType) {
      ...
      default:
        return <Input type={config.type} placeholder={config.placeholder} {...field} className='w-full' />
    }
  }
        ...
        */}
        <div className='grid grid-cols-1 gap-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 lg:gap-6'>
              ...
              <FormField
                ...
                render={({ field }) => (
                  ...
                  <FormItem className='min-w-0'>
                    <FormLabel className='text-sm'>{config.label ?? name}</FormLabel>
                    <FormControl>{renderFormField(field, name, config, fieldSchema as z.ZodTypeAny)}</FormControl>
          ...
          <div className='mt-4 flex w-full flex-col justify-center gap-4 md:col-span-2 md:mt-0 md:flex-row md:justify-end lg:col-span-4'>
            <Button variant='outline' className='w-full md:w-auto' onClick={() => form.reset()}>
              {translateFn('refresh')}
            </Button>
